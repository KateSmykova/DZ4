...
def get_notmy_posts(token):
    ...
    if g:
        ...
        logging.error('Страница не открылась')
def createpost(token):
    ...
    p = requests.post('https://test-stand.gb.ru/gateway/posts', headers={'X-Auth-Token': token},
                      data={'title': 'Новый пост',
                            ...
                            'content': 'Добавить еще один тест, в котором создается новый пост,'
                                       'а потом проверить его наличие на сервере по полю “описание”.'})
    if p:
        return p.json()
    else:
